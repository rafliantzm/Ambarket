...
import 'package:ambarket_mobile/features/home/presentation/widgets/animated_promo_hero_card.dart';
...
void main() {
  group('Home Screen Tests', () {
    testWidgets('Renders all discovery mode sections when search is empty', (
      WidgetTester tester,
    ) async {
      ...
      expect(
        find.byType(HomeSearchHeader, skipOffstage: false),
        findsOneWidget,
      );
      expect(
        find.byType(AnimatedPromoHeroCard, skipOffstage: false),
        findsOneWidget,
      );
      expect(
        find.byType(HomeQuickActions, skipOffstage: false),
        findsOneWidget,
      );
      expect(
        find.byType(HomeCategoryStrip, skipOffstage: false),
        findsOneWidget,
      );
      expect(find.byType(HomePromoBanner, skipOffstage: false), findsOneWidget);
      expect(
        find.byType(HomeProductSection, skipOffstage: false),
        findsWidgets,
      ); // Multiple product sections
    });

    testWidgets('Renders search mode when query is not empty', (
      WidgetTester tester,
    ) async {
