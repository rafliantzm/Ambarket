...
void main() {
  ...
  for (final path in files) {
    ...
    if (path.contains('notifications_screen.dart')) {
      ...
      content = content.replaceAll('_buildNotificationItem(notification)', '_buildNotificationItem(context, notification)');
    }

    // In product_condition_section.dart and product_info_section.dart
    if (path.contains('product_condition_section.dart') || path.contains('product_info_section.dart')) {
      content = content.replaceAll('Widget _buildInfoRow(String label, String value, {bool highlight = false}) {', 'Widget _buildInfoRow(BuildContext context, String label, String value, {bool highlight = false}) {');
